...
Calculator.prototype.updateTotal = function () {
    ...
    // products
    if (this.form.products.value !== "") {
        total = total + (parseInt(this.form.products.value, 10) * this.prices.products);
    }

    // orders
    if (this.form.orders.value !== "") {
        total = total + (parseInt(this.form.orders.value, 10) * this.prices.orders);
    }

    // package
    var selectedPackage = this.form.package.getAttribute("data-value");
    if (selectedPackage !== null && selectedPackage !== "") {
        total = total + this.prices.package[selectedPackage];
    }

    // accounting
    if (this.form.accounting.checked) {
        total = total + this.prices.accounting;
    }

    // terminal
    if (this.form.terminal.checked) {
        total = total + this.prices.terminal;
    }

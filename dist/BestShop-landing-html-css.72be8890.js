...
/* Summary: pokaži + posodobi vrstico */ Calculator.prototype.updateSummary = function(id, calcText, totalValue) {
    ...
    if (calcEl) calcEl.innerText = calcText || "";
    priceEl.innerText = "$" + totalValue;
};
/* Summary: skrij vrstico */ Calculator.prototype.hideSummary = function(id) {
    const item = this.summary.list.querySelector('[data-id="' + id + '"]');
    item.classList.remove("open");
};
/* Validacija: pozitivno celo število ali prazno */ Calculator.prototype.getValidInteger = function(value) {
    if (value === "") return "";
    const number = parseInt(value, 10);
    if (isNaN(number) || number < 1) return "";
    return String(number);
};
/* Total */ Calculator.prototype.updateTotal = function() {
    const show = this.summary.list.querySelectorAll(".open").length > 0;
    if (!show) {
        this.summary.total.container.classList.remove("open");
        return;
    }
    const productsSum = this.form.products.value === "" ? 0 : parseInt(this.form.products.value, 10) * this.prices.products;
    const ordersSum = this.form.orders.value === "" ? 0 : parseInt(this.form.orders.value, 10) * this.prices.orders;
...
/* Inputi: products/orders */ Calculator.prototype.inputEvent = function(e) {
...
/* Dropdown: package */ Calculator.prototype.selectEvent = function(e) {
...
/* Checkboxi */ Calculator.prototype.checkboxEvent = function(e) {
    const checkbox = e.currentTarget;
    const id = checkbox.id;
    if (checkbox.checked) this.updateSummary(id, "", this.prices[id]);
    else this.hideSummary(id);
    this.updateTotal();
};
/* Eventi */ Calculator.prototype.addEvents = function() {
...
/* Start */ document.addEventListener("DOMContentLoaded", function() {
